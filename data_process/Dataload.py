...
class Dataloader():
    ...
    def get_audio_data(self, audio_feature_path, padding=False):
        ...
        if self.one_only:
            df = self.df[self.df['gender'] == [s[0]
                                               for s in self.df['sentence']]]
        else:
            df = self.df
        audio_data = []
        for index, row in df.iterrows():
            data_path = os.path.join(
                audio_feature_path, 'Session'+str(row['Session'])+'/'+row['dialog']+'/'+index+'.npy')
    ...
    def get_motion_data(self, motion_path, padding=False):
        ...
        for index, row in df.iterrows():
            data_path = os.path.join(
                motion_path, 'Session'+str(row['Session'])+'/'+row['dialog']+'.npy')
            with open(data_path, 'rb') as f:
                data = np.load(f)
    ...
    def get_emo_label(self):
        if self.one_only:
            ...
            df = self.df
        emo_map = {'neu': 0, 'fru': 1, 'ang': 2, 'sad': 3, 'exc': 4,
                   'sur': 5, 'hap': 6, 'fea': 7, 'dis': 8, 'oth': 9, 'xxx': -1}
